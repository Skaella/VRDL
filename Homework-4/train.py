...
def train():
    ...
    parser.add_argument(
        "--batch_size",
        type=int,
        default=1)  # Small for laptop VRAM
    ...
    for epoch in range(args.epochs):
        ...
        mean_t = torch.tensor([0.485, 0.456, 0.406]).view(
            1, 3, 1, 1).to(device)
        ...
        with torch.no_grad():
            for val_batch in val_loader:
                ...
                # Un-normalize both back to [0, 1] to calculate accurate
                # mathematical PSNR
                val_restored_unnorm = torch.clamp(
                    (val_restored * std_t) + mean_t, 0.0, 1.0)
                val_clean_unnorm = torch.clamp(
                    (val_clean * std_t) + mean_t, 0.0, 1.0)
        ...
        print(
            f"Epoch {epoch+1} Summary - Train Loss: {avg_loss:.4f} | Val PSNR: {avg_val_psnr:.2f} dB")

        # Append real values to generates an accurate graph
        metrics_history.append({"epoch": epoch +
                                1, "loss": avg_loss, "val_psnr": avg_val_psnr})
        ...
        if (epoch + 1) % 10 == 0:
            ckpt_path = os.path.join(
                args.ckpt_dir, f"promptir_epoch_{epoch+1}.pth")
