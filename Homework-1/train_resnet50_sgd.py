...
def main():
    # DIRECTORY SETUP
    FOLDER_NAME = "sgd_results"
    OUTPUT_DIR = os.path.join("outputs", FOLDER_NAME)
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    LOG_PATH = os.path.join(OUTPUT_DIR, "logs_sgd.csv")
    MODEL_PATH = os.path.join(OUTPUT_DIR, "resnet50_sgd_best.pth")
    ...
    plot_loss_curve(tracker, suffix="sgd", output_dir=OUTPUT_DIR)
    plot_accuracy_curve(tracker, suffix="sgd", output_dir=OUTPUT_DIR)
    plot_confusion_matrix(
        model, val_loader, device, suffix="sgd", output_dir=OUTPUT_DIR
    )
